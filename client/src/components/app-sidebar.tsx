...
const AppSidebar = ({ userType }: AppSidebarProps) => {
  ...
  const navLinks = [...roleSpecificLinks];
  ...
  return (
    <>
      ...
      <Sidebar
        collapsible="icon"
        className={cn(
          "fixed left-0 bg-background/95 backdrop-blur-sm border-r border-border transition-transform duration-300 ease-in-out z-50",
          isMobile
            ? open
      ...
      >
        <SidebarHeader className="border-b border-border pb-2 pt-1">
          ...
          >
            ...
            <button
              className={cn(
                "hover:bg-muted p-2 rounded-md transition-colors duration-200 shrink-0",
                (open || isMobile) && "ml-2"
              )}
              ...
              return (
                <SidebarMenuItem key={link.href}>
                  <SidebarMenuButton
                    asChild
                    className={cn(
                      "flex items-center my-0.5 rounded-md transition-colors duration-150 ease-in-out group",
                      (open || isMobile) &&
